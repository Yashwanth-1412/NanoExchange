...
struct LiveOrder { ClientId clientId; OrderId clientOrderId; };
...
std::vector<SimOrder> genRealistic(size_t n, uint64_t seed = 42) {
    ...
    for (auto& p : PARTICIPANTS) for (int w = 0; w < p.weight; ++w) pool.push_back(&p);
    ...
    for (size_t i = 0; i < n; ++i) {
        if (i % 500 == 0) {
            drift += driftD(gen); drift = std::clamp(drift, -200, 200);
            mid += drift; mid = std::clamp(mid, (Price)8000, (Price)12000);
        }
        ...
        if (doCancel) {
            ...
            size_t idx = pk(gen); LiveOrder t = live[idx];
            ...
            Price price = p->aggressive ? (side == Side::BUY ? mid + edgeD(gen) : mid - edgeD(gen)) : (side == Side::BUY ? mid - edgeD(gen) : mid + edgeD(gen));
            price = std::max((Price)1, price);
            OrderType type = pct(gen) < 10 ? OrderType::FillAndKill : OrderType::GoodTillCancel;
            OrderId oid = nextId++; ClientId cid = p->id;
            orders.push_back({ cid, oid, type, price, qtyD(gen), side, false, 0 });
            if (type == OrderType::GoodTillCancel) {
                live.add(cid, oid);
                if (live.size() > 1'000'000) for (int k = 0; k < 100000; ++k) live.erase(0);
            }
        }
    }
    return orders;
...
std::vector<SimOrder> genLargeWorkingSet(size_t n, uint64_t seed = 1) {
    ...
    std::uniform_int_distribution<int> pct(0, 99);
    std::uniform_int_distribution<int> coin(0, 1);
    std::vector<SimOrder> orders; LiveSet live; orders.reserve(n);
    ...
    for (size_t i = 0; i < n; ++i) {
        ...
        if (doCancel) {
            ...
            Side side = coin(gen) ? Side::BUY : Side::SELL;
            Price price = priceD(gen); OrderId oid = nextId++; ClientId cid = 1001 + (oid % 10);
            orders.push_back({ cid, oid, OrderType::GoodTillCancel, price, qtyD(gen), side, false, 0 });
            live.add(cid, oid);
            if (live.size() > 800'000) for (int k = 0; k < 50000; ++k) live.erase(0);
        }
    }
    return orders;
}
...
std::vector<SimOrder> genColdCancels(size_t n, uint64_t seed = 2) {
    ...
    std::uniform_int_distribution<int> coin(0, 1);
    std::vector<SimOrder> orders; std::vector<LiveOrder> live; orders.reserve(n * 2);
    OrderId nextId = 400000;

    for (size_t i = 0; i < n; ++i) {
        Side side = coin(gen) ? Side::BUY : Side::SELL; Price price = priceD(gen);
        OrderId oid = nextId++; ClientId cid = 1001 + (oid % 10);
        orders.push_back({ cid, oid, OrderType::GoodTillCancel, price, qtyD(gen), side, false, 0 });
        live.push_back({ cid, oid });
    }
    std::shuffle(live.begin(), live.end(), gen);
    for (auto& lo : live) orders.push_back({ lo.clientId, 0, OrderType::GoodTillCancel, 0, 0, Side::BUY, true, lo.clientOrderId });
    return orders;
}
...
std::vector<SimOrder> genDeepBook(size_t n, uint64_t seed = 3) {
    ...
    std::uniform_int_distribution<int> pct(0, 99);
    std::uniform_int_distribution<int> coin(0, 1);
    std::vector<SimOrder> orders; orders.reserve(n + 10000);
    OrderId nextId = 500000;

    for (int p = 10001; p <= 15000; ++p) orders.push_back({ 1001, nextId++, OrderType::GoodTillCancel, (Price)p, 50, Side::SELL, false, 0 });
    for (int p = 9999; p >= 5000; --p) orders.push_back({ 1002, nextId++, OrderType::GoodTillCancel, (Price)p, 50, Side::BUY, false, 0 });

    size_t remaining = n > 10000 ? n - 10000 : 0;
    std::uniform_int_distribution<Price> aggrBuy(10001, 15000); std::uniform_int_distribution<Price> aggrSell(5000, 9999);
    std::uniform_int_distribution<Price> passAsk(10001, 15000); std::uniform_int_distribution<Price> passBid(5000, 9999);

    for (size_t i = 0; i < remaining; ++i) {
        if (pct(gen) < 70) {
            Side side = coin(gen) ? Side::BUY : Side::SELL; Price price = (side == Side::BUY) ? aggrBuy(gen) : aggrSell(gen);
            orders.push_back({ 1003, nextId++, OrderType::GoodTillCancel, price, 50, side, false, 0 });
            if (side == Side::BUY) orders.push_back({ 1001, nextId++, OrderType::GoodTillCancel, price, 50, Side::SELL, false, 0 });
            else orders.push_back({ 1002, nextId++, OrderType::GoodTillCancel, price, 50, Side::BUY, false, 0 });
        } else {
            Side side = coin(gen) ? Side::BUY : Side::SELL; Price price = (side == Side::BUY) ? passBid(gen) : passAsk(gen);
...
std::vector<SimOrder> genFragmented(size_t n, uint64_t seed = 4) {
    ...
    std::uniform_int_distribution<Price> priceD(1, 240000);
    std::uniform_int_distribution<int> coin(0, 1);
    std::vector<SimOrder> orders; orders.reserve(n * 2);
    OrderId nextId = 600000;

    for (size_t i = 0; i < n; ++i) {
        Price price = priceD(gen); Side side = coin(gen) ? Side::BUY : Side::SELL;
        OrderId oid = nextId++; ClientId cid = 1001 + (oid % 10);
...
std::vector<SimOrder> genSweeps(size_t n, uint64_t seed = 5) {
    ...
    std::vector<SimOrder> orders; orders.reserve(n + 100);
    OrderId nextId = 700000; int nextBurst = calmLen(gen); int burstLeft = 0;

    for (size_t i = 0; i < n; ++i) {
        if (burstLeft == 0 && (int)i >= nextBurst) { burstLeft = burstSize(gen); nextBurst = (int)i + calmLen(gen); }
        if (burstLeft > 0) {
        ...
        Side side = coin(gen) ? Side::BUY : Side::SELL; Price price = makerPrice(gen);
        price = (side == Side::BUY) ? price - 5 : price + 5;
        OrderId oid = nextId++; ClientId cid = 1001 + (oid % 4);
...
std::vector<SimOrder> genDeepQueue(size_t n, uint64_t seed = 6) {
    ...
    std::shuffle(live.begin(), live.end(), gen);
    for (size_t i = 0; i < buildCount; ++i) {
        orders.push_back({ live[i].clientId, 0, OrderType::GoodTillCancel, 0, 0, Side::BUY, true, live[i].clientOrderId });
    }
...
// ============================================================
std::vector<SimOrder> genFlickeringBBO(size_t n, uint64_t seed = 7) {
    std::vector<SimOrder> orders;
    orders.reserve(n);
    ...
    for (size_t i = 0; i < loops; ++i) {
        ...
        if (i % 10 == 0) basePrice = 10000; else basePrice++;
    }
...
// ============================================================
std::vector<SimOrder> genFlashCrash(size_t n, uint64_t seed = 8) {
    std::vector<SimOrder> orders;
    orders.reserve(n);
...
bool runCorrectness(Book* book, std::string_view tag) {
    std::cout << "  " << col::BLU << std::left << std::setw(38) << tag << col::RST << " ... "; std::cout.flush();
    try {
    ...
    } catch (std::exception& e) { std::cout << col::RED << "FAIL — " << e.what() << col::RST << '\n'; return false; } 
      catch (...) { std::cout << col::RED << "FAIL" << col::RST << '\n'; return false; }
...
BenchResult runAllPhases(const std::vector<std::vector<SimOrder>>& phases, const std::vector<SimOrder>& warmup) {
    ...
    BenchResult result; result.name = Book::NAME; result.correct = true;

    for (auto& phaseOrders : phases) {
        auto* book = new Book(POOL_SIZE, 1, nullptr, nullptr);
        for (auto& o : warmup) {
            if (o.isCancel) book->cancelOrder(1, o.clientId, o.cancelTargetId);
            else book->addOrder(1, o.clientId, o.clientOrderId, o.type, o.price, o.qty, o.side);
        }

        std::vector<double> samples; samples.reserve(phaseOrders.size());
        long long totalNs = 0;
        ...
        for (const auto& o : phaseOrders) {
            ...
            samples.push_back(ns); totalNs += static_cast<long long>(ns);
        }
...
void printPhaseTable(const BenchResult& r, const std::vector<std::string>& phaseNames) {
    ...
    std::cout << col::DIM << "  " << std::left << std::setw(12) << "Phase" << std::right << std::setw(9) << "avg" << std::right << std::setw(9) << "p50" << std::right << std::setw(9) << "p99" << std::right << std::setw(10) << "p999" << std::right << std::setw(10) << "max" << std::right << std::setw(11) << "M ops/s" << std::right << std::setw(7) << "ms" << col::RST << '\n'; line();

    for (size_t i = 0; i < r.phases.size(); ++i) {
        const auto& s = r.phases[i];
        std::cout << "  " << std::left << std::setw(12) << phaseNames[i];
        auto ns = [](double v) -> std::string { std::ostringstream ss; ss << std::fixed << std::setprecision(1) << v << "ns"; return ss.str(); };
        bool spikey = s.p99 > s.avg * 3.0;

        std::cout << col::YLW << std::right << std::setw(9) << ns(s.avg) << col::RST;
        std::cout << std::right << std::setw(9) << ns(s.p50);
        if (spikey) std::cout << col::RED; std::cout << std::right << std::setw(9) << ns(s.p99); if (spikey) std::cout << col::RST;
        std::cout << col::MAG << std::right << std::setw(10) << ns(s.p999) << col::RST;
        std::cout << col::RED << std::right << std::setw(10) << ns(s.max) << col::RST;
        std::cout << col::GRN << std::right << std::setw(9) << std::fixed << std::setprecision(1) << s.throughputM << "M" << col::RST;
...
void printSummaryTable(const std::vector<BenchResult>& results, const std::vector<std::string>& phaseNames) {
    ...
    for (auto& r : results) for (size_t p = 0; p < nPhases; ++p) { bestAvg[p] = std::min(bestAvg[p], r.phases[p].avg); bestP99[p] = std::min(bestP99[p], r.phases[p].p99); }

    line("="); std::cout << col::BOLD << col::WHT << "  SUMMARY — avg ns / p99 ns  (* = fastest)\n" << col::RST; line("=");
    std::cout << col::DIM << "  " << std::left << std::setw(32) << "Variant";
    for (auto& ph : phaseNames) std::cout << std::right << std::setw(15) << ph;
    std::cout << col::RST << '\n'; line();
    ...
    for (auto& r : results) {
        ...
        for (size_t p = 0; p < nPhases; ++p) {
            ...
            std::string s = std::to_string((int)r.phases[p].avg) + "/" + std::to_string((int)r.phases[p].p99) + (best ? "*" : " ");
            if (best) std::cout << col::GRN << col::BOLD; else std::cout << col::YLW;
...
int main() {
    ...
    std::vector<std::string> phaseNames = {
        "Realistic", "LargeWS", "ColdCxl", "DeepBook", "Fragmntd", "Sweeps", "MemeStock", "Pennying", "FlshCrsh"
    };
    ...
    std::vector<std::vector<SimOrder>> phases = {
        genRealistic      (N, 42),
        genLargeWorkingSet(N,  1),
        genColdCancels    (N,  2),
        genDeepBook       (N,  3),
        genFragmented     (N,  4),
        genSweeps         (N,  5),
        genDeepQueue      (N,  6),
        genFlickeringBBO  (N,  7),
        genFlashCrash     (N,  8),
    };

    std::cout << "  Event counts: ";
    for (size_t i = 0; i < phases.size(); ++i) std::cout << phaseNames[i] << "=" << phases[i].size() << " ";
    std::cout << "\n";

    auto warmup = genRealistic(NW, 99);

    std::cout << '\n'; line(); std::cout << col::BOLD << "  CORRECTNESS\n" << col::RST; line();
    bool allOk = true;
    { auto* b = new MEOrderBook_PointerPool(500000,1,nullptr,nullptr); allOk &= runCorrectness(b, MEOrderBook_PointerPool::NAME); delete b; }
    { auto* b = new MEOrderBook_DirectVector(500000,1,nullptr,nullptr); allOk &= runCorrectness(b, MEOrderBook_DirectVector::NAME); delete b; }
    { auto* b = new MEOrderBook_PointerPreAlloc(500000,1,nullptr,nullptr); allOk &= runCorrectness(b, MEOrderBook_PointerPreAlloc::NAME); delete b; }
    { auto* b = new MEOrderBook_StdMap(500000,1,nullptr,nullptr); allOk &= runCorrectness(b, MEOrderBook_StdMap::NAME); delete b; }

    if (!allOk) { std::cout << col::RED << "\n  Correctness failure — aborting.\n" << col::RST; return 1; }
    std::cout << col::GRN << "  All correct.\n" << col::RST;

    std::cout << '\n'; line(); std::cout << col::BOLD << "  BENCHMARKS\n" << col::RST; line();

    std::vector<BenchResult> results;
    auto runVariant = [&](auto* proto) {
        using Book = std::remove_pointer_t<decltype(proto)>;
        delete proto;
        std::cout << col::BLU << "  Running " << Book::NAME << " ...\n" << col::RST; std::cout.flush();
    ...
    std::cout << '\n'; line(); std::cout << col::BOLD << "  PHASE GUIDE\n" << col::RST; line();
    std::cout << col::DIM
        << "  Realistic   — mixed flow, baseline\n"
