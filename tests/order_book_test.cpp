...
private:
    ...
    void generateAll() {
        ...
        for (size_t i = 0; i < count_; ++i) {
            ...
            bool doCancel = !liveOrders_.empty() &&
            ...
            if (doCancel) {
                ...
                if (liveOrders_.size() > 50000)
                    liveOrders_.erase(liveOrders_.begin(),
                                      liveOrders_.begin() + 10000);
            }
        }
    }
};
...
void runCorrectnessTests(MEOrderBook* book) {
    ...
    auto ok = [](std::string_view msg) {
        std::cout << col::GRN << "  [OK] " << col::RST << msg << '\n';
    };
    ...
    std::cout << col::GRN << col::BOLD
              << "  ALL CORRECTNESS SCENARIOS PASSED\n"
              << col::RST;
    line("=");
    std::cout << '\n';
}
...
void runThroughputBenchmark(MEOrderBook* book, size_t numOrders) {
    ...
    for (auto& o : warmupStream.orders()) {
        if (o.isCancel)
            ...
            book->addOrder(1, o.clientId, o.clientOrderId,
                           o.type, o.price, o.qty, o.side);
    }
    ...
    std::cout << "  Add orders           : " << addCount    << " (" << std::fixed << std::setprecision(1) << addFraction*100    << "%)\n";
    std::cout << "  Cancel orders        : " << cancelCount << " (" << cancelFraction*100 << "%)\n";
    std::cout << "  Total time           : " << ms          << " ms\n";
    std::cout << col::YLW
              << "  Avg latency          : " << std::setprecision(2) << nsPerOrder   << " ns/order\n"
              << col::RST;
    std::cout << col::GRN
              << "  Throughput           : " << std::setprecision(0) << ordersPerSec << " orders/sec\n"
              << col::RST;
    line("-");
    ...
    for (auto& p : PARTICIPANTS) {
        ...
        std::cout << "    " << std::left  << std::setw(24) << p.name
                  << std::right << std::setw(7) << cnt << " events\n";
    }
    std::cout << col::RST << '\n';
