...
int main() {
    ...
    check("OUCH decode preserves 1/10000 price",
          decodeOuch(reinterpret_cast<const char*>(&order), 1, token_manager, request) &&
          request.price_ == 1'001'234);
    ...
    check("ITCH encode preserves 1/10000 price",
          encoded_size == sizeof(quantlink::itch::AddOrder) &&
          quantlink::itch::swap32(encoded.price) == 1'001'234U);
