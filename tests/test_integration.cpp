...
private:
    ...
    struct LiveOrder { ClientId client_id_; OrderId order_id_; TickerId ticker_id_; };
...
public:
    explicit RandomOrderGenerator(uint64_t seed = 42) : rng_(seed) {
...
struct Stats {
    ...
    std::atomic<uint64_t> market_updates_sent{0};
};
...
int main() {
    ...
    std::thread stats_thread([&]() {
        while (g_running.load(std::memory_order_acquire)) {
            ...
            std::cout << "\r  Orders: "   << stats.orders_sent.load()
                      << "  Cancels: "    << stats.cancels_sent.load()
                      << "  Responses: "  << stats.responses_received.load()
                      << "  MktUpdates: " << stats.market_updates_sent.load()
                      << "    " << std::flush;
        }
    });
