...
void send(ClientRequestType action, ClientId clientId, OrderId clientOrderId, TickerId tickerId, OrderType type,
...
void sendNew(ClientId clientId, OrderId clientOrderId, TickerId tickerId, OrderType type, Side side, Price price,
             Quantity qty) {
...
void sendCancel(ClientId clientId, OrderId clientOrderId, TickerId tickerId, Quantity qty = 0) {
    send(ClientRequestType::CANCEL, clientId, clientOrderId, tickerId, OrderType::GoodTillCancel, Side::BUY, 0, qty);
}

void sendReplace(ClientId clientId, OrderId clientOrderId, OrderId newClientOrderId, TickerId tickerId, OrderType type,
                 Side side, Price price, Quantity qty) {
...
void test_full_fill() {
    ...
    CHECK(resps[1].match_id_ == resps[2].match_id_ && resps[1].match_id_ == updates[0].match_id_,
          "private and public executions share match ID");
...
void test_multi_level_sweep() {
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        if (i > 0)
            CHECK(updates[i - 1].match_id_ != updates[i].match_id_, "separate fills have different match IDs");
    }
}
...
void test_partial_cancel_order() {
    ...
    CHECK(resps.size() == 1 && resps[0].status_ == ResponseType::CANCELED, "remaining quantity cancels successfully");
...
void test_replace_order() {
    ...
    CHECK(resps.size() == 1 && resps[0].status_ == ResponseType::MODIFIED, "replacement response is MODIFIED");
    CHECK(resps[0].client_order_id_ == 43 && resps[0].price_ == 101 && resps[0].qty_ == 12,
          "replacement uses new client ID, price, and quantity");
    CHECK(updates.size() == 1 && updates[0].type_ == UpdateType::MODIFY, "public book receives ITCH replace update");
    CHECK(updates[0].market_order_id_ != updates[0].new_order_id_ && updates[0].price_ == 101 && updates[0].qty_ == 12,
          "public replace retires old order ID");

    sendCancel(1, 42, 0);
    resps = drainResponses();
    CHECK(resps.size() == 1 && resps[0].status_ == ResponseType::CANCEL_REJECTED, "old client order ID is retired");

    sendCancel(1, 43, 0);
    resps   = drainResponses();
    updates = drainMarketUpdates();
    CHECK(resps.size() == 1 && resps[0].status_ == ResponseType::CANCELED && resps[0].leaves_qty_ == 12,
          "replacement can be canceled by new client order ID");
