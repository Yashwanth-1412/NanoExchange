...
class SnapshotStreamer {
  ...
  private:
    ...
    auto bakeSnapshot() noexcept -> void {
        ...
        for (size_t ticker_id = 0; ticker_id < maxTickers_; ++ticker_id) {
            ...
            for (const auto& [order_id, order] : shadow_book_[ticker_id]) {
                ...
                if (size > 0)
                    snapshot_buf_.insert(snapshot_buf_.end(), buf, buf + size);
            }
        }
    ...
    auto serveReplay(int fd, uint64_t client_seq) noexcept -> void {
        ...
        const uint64_t oldest_replayable = last_seq_num_ > REPLAY_CAPACITY ? last_seq_num_ - REPLAY_CAPACITY : 0;
    ...
    auto handleClient(int fd) noexcept -> void {
        ...
        if (request[0] == 'R') {
            if (!recvExact(fd, request + 1, sizeof(uint64_t)))
                return;
    ...
    auto serveRequests() noexcept -> void {
        while (true) {
            ...
            if (client_fd < 0)
                return; // EAGAIN or error — no more pending connections
  ...
  public:
    SnapshotStreamer(SPSCQueue<SnapshotUpdate>* snapshotUpdates, quantlink::Logger* logger, size_t maxTickers,
        ...
        ASSERT(quantlink::setNonBlocking(snapshot_tcp_fd_),
               "SnapshotStreamer: failed to make TCP snapshot socket nonblocking");
    }
    ...
    auto applyToBook(const MEMarketUpdate* update) -> void {
        ...
        case UpdateType::TRADE: {
            ...
            if (it != order_map.end()) {
                if (it->second.qty_ <= update->qty_) {
                    ...
                    it->second.qty_ -= update->qty_; // Partially filled — reduce qty
            } else {
        ...
        if (update->type_ == UpdateType::ADD || update->type_ == UpdateType::CANCEL ||
            update->type_ == UpdateType::MODIFY || update->type_ == UpdateType::TRADE) {
            dirty_ = true;
        }
    }
};
