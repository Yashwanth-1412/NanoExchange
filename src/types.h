...
struct MEClientRequest {
    ...
    MEClientRequest(ClientRequestType action, ClientId client_id, TickerId ticker_id, OrderId client_order_id,
...
struct MEClientResponse {
    ...
    // Parameterized constructor
    MEClientResponse(ClientId client_id, TickerId ticker_id, OrderId client_order_id, OrderId market_order_id,
                     Side side, Price price, Quantity qty, OrderType type, ResponseType status, Quantity executed_qty,
                     Price execution_price, Quantity leaves_qty, OrderId match_id = OrderId_INVALID)
        : client_id_(client_id),
          client_order_id_(client_order_id),
...
struct MEMarketUpdate {
    ...
    MEMarketUpdate(TickerId ticker_id, OrderId market_order_id, UpdateType type, Side side, Price price, Quantity qty,
                   OrderId new_order_id = OrderId_INVALID, OrderId match_id = OrderId_INVALID)
        : ticker_id_(ticker_id),
          market_order_id_(market_order_id),
          type_(type),
          side_(side),
...
struct MEOrder {
    ...
    MEOrder(TickerId tickerId, ClientId clientId, OrderId clientOrderId, OrderId marketOrderId, OrderType orderType,
            Price price, Quantity initialQuantity, Side side, const OrderToken& orderToken = {},
